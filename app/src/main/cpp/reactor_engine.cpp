    ...
    if(state_.scram_active) {
        ...
        for(int i=0;i<NUM_CONTROL_RODS;i++) {
            if(state_.rod_pos[i]>0.01) {
                state_.rod_pos[i]=std::max(0.0,state_.rod_pos[i]-(1.0/18.0)*dt);
                state_.rod_moving[i]=true; all_in=false;
            } else { state_.rod_pos[i]=0; state_.rod_moving[i]=false; }
        }
...
void ReactorEngine::triggerLevel16Sequence(double T, double dt) {
    ...
    if(T>285.0&&!state_.az5_pressed){
        ...
        state_.az5_pressed=true;state_.scram_active=true;scram_time_=T;state_.rod_target.fill(0.0);
    }
